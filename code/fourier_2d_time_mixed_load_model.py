...
def read_train_data(input_dir,ntrain):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(".mat") and ( "V1e-04" in filename or "V1e-05" in filename):
            ...
            train_u.append(reader.read_field('u')[:ntrain, ::sub, ::sub, T_in:T + T_in])
            print("finished " + filename)
    x_train_mixed = torch.cat([item for item in train_a], 0)
    y_train_mixed = torch.cat([item for item in train_u], 0)
...
path = '../models/ns/mixed_100_shot_e-4_e-5'+str(ntrain)+'_ep' + str(epochs) + '_m' + str(modes) + '_w' + str(width)
path_model = path
