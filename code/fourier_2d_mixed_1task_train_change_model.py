...
class FNO2d_P_1layer(nn.Module):
    def __init__(self, modes1, modes2, width, task_num):
        super(FNO2d_P_1layer, self).__init__()
        ...
        self.padding = 9  # pad the domain if input is non-periodic
        #self.fc0 = nn.Linear(3, self.width)  # input channel is 3: (a(x, y), x, y)
        self.fc0 = nn.ModuleList([nn.Linear(3, self.width) for i in range(task_num + 1)])
...
class FNO2d_P_Q2layer(nn.Module):
    def __init__(self, modes1, modes2, width, task_num):
        super(FNO2d_P_Q2layer, self).__init__()
...
model_name = train_ratio+'Q_2layer_train_task1-17_1000_test_3_800_with_norm_train_model'
